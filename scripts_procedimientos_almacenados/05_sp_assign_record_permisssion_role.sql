...
-- Description: This storage procedure allows you to assign or revoke permissions on a specific record in an entity (for example, a role on a specific branch).
---------------------------------------
...
BEGIN
    ...
    BEGIN TRY
        ...
        USING (SELECT @RoleID AS role_id, @EntityCatalogID AS entitycatalog_id, @PermissionID AS permission_id, @RecordID AS perrc_record) AS source
        ...
        WHEN NOT MATCHED THEN
            INSERT (role_id, entitycatalog_id, permission_id, peusr_record, peusr_include)
            VALUES (source.role_id, source.entitycatalog_id, source.permission_id, source.perrc_record, @Include);
